...
export const ServiceHowItWorks = ({
  ...
  heading,
  steps,
}: ServiceHowItWorksProps): JSX.Element => {
    ...
    >
      ...
      <div className="flex flex-col items-center relative w-full max-w-[1280px]">
        {steps.map((step, index) => (
          <article
            key={step.number}
            className="flex flex-row lg:flex-row items-start lg:items-center justify-center gap-4 md:gap-6 lg:gap-14 relative self-stretch w-full"
          >
            ...
            {step.alignment === "left" && (
              <div className="flex flex-col items-start lg:items-end gap-6 md:gap-8 lg:gap-[var(--3-spacing-spacing-3xl)] pt-2 md:pt-[var(--3-spacing-spacing-md)] pb-8 md:pb-[var(--3-spacing-spacing-4xl)] px-0 relative w-full lg:w-[500px] order-2 lg:order-1">
                ...
                {/* Text content - LEFT-ALIGNED on mobile, RIGHT-ALIGNED on desktop for left-side cards */}
                <div className="flex flex-col items-start lg:items-end gap-2 md:gap-[var(--3-spacing-spacing-md)] relative self-stretch w-full">
                  <h3 className="relative self-stretch [font-family:'Inter',Helvetica] font-semibold text-incorpify-primary text-xl md:text-2xl text-left lg:text-right tracking-[0] leading-7 md:leading-[30px]">
                    {step.title}
                  </h3>

                  <p className="relative self-stretch [font-family:'Inter',Helvetica] font-normal text-gray-600 text-sm md:text-base text-left lg:text-right tracking-[0] leading-5 md:leading-6">
                    {step.description}
                  </p>
                </div>
              </div>
            ...
            <div className="flex flex-col w-12 items-center relative self-stretch order-1 lg:order-2 flex-shrink-0">
              <div
                className={`${
                  activeSteps[index]
                    ? "z-[1] border-incorpifyprimary bg-[linear-gradient(90deg,rgba(29,24,52,1)_0%,rgba(86,71,154,1)_100%)]"
                    : "bg-white border-gray-200"
                } flex flex-col w-10 h-10 md:w-12 md:h-12 items-center justify-center gap-3 p-3 relative ${
                  index > 0 ? "z-[1]" : ""
                } rounded-[99px] border-[3px] border-solid flex-shrink-0 transition-all duration-500`}
              >
                <span
                  className={`relative w-fit ${
                    activeSteps[index]
                      ? "[font-family:'Inter',Helvetica] font-semibold text-white text-sm md:text-base"
                      : "[font-family:'Inter',Helvetica] font-semibold text-gray-400 text-sm md:text-base"
                  } tracking-[0] leading-5 md:leading-6 whitespace-nowrap`}
                >
                  {step.number}
                </span>
              </div>

              {/* Connector line - vertical on all screen sizes */}
              {index < steps.length - 1 && (
                <div className="flex flex-col w-1 items-start relative flex-1 grow -mt-4 z-0 rounded-[var(--3-spacing-spacing-md)] overflow-hidden">
            ...
            {step.alignment === "right" && (
              <div className="flex flex-col items-start gap-6 md:gap-8 lg:gap-[var(--3-spacing-spacing-3xl)] pt-2 md:pt-[var(--3-spacing-spacing-md)] pb-8 md:pb-[var(--3-spacing-spacing-4xl)] px-0 relative w-full lg:w-[500px] order-2 lg:order-3">
                ...
                {/* Text content - LEFT-ALIGNED on all screen sizes for right-side cards */}
                <div className="flex flex-col items-start gap-2 md:gap-[var(--3-spacing-spacing-md)] relative self-stretch w-full">
                  <h3 className="relative self-stretch [font-family:'Inter',Helvetica] font-semibold text-incorpify-primary text-xl md:text-2xl text-left tracking-[0] leading-7 md:leading-[30px]">
                    {step.title}
                  </h3>

                  <p className="relative self-stretch [font-family:'Inter',Helvetica] font-normal text-gray-600 text-sm md:text-base text-left tracking-[0] leading-5 md:leading-6">
                    {step.description}
                  </p>
                </div>
              </div>
            ...
            {step.alignment === "left" && (
              <div
                className="hidden lg:flex flex-col w-[500px] items-start gap-[var(--3-spacing-spacing-3xl)] pt-[var(--3-spacing-spacing-md)] pb-[var(--3-spacing-spacing-4xl)] px-0 relative order-4"
                aria-hidden="true"
              />
            )}
          </article>
